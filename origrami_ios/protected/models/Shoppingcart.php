...
class Shoppingcart extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('instagram_id', 'required'),
			array('instagram_id, coupon, status', 'numerical', 'integerOnly'=>true),
			array('fbshare, twshare', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, instagram_id, coupon, fbshare, twshare, status', 'safe', 'on'=>'search'),
		);
	}
