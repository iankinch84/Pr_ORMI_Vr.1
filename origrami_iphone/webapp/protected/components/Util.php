...
class Util{
    ...
    */
     ...
     * @return array() Shoppingcart Information
     */
     ...
     * get Object Value
     * 
     * @param variable $_array
     * @return object
     */
    public static function getObjectValue($_array) {
        return self::isVariableEmpty($_array) ? null : $_array;
    }
    
    /**
     * Pre User Registration Process
     * 
     * @param array() $_user
     * @return Object
     */
    public static function preUserRegistration($_user){
        if ( self::isVariableEmpty($_user["email"]) && self::isVariableEmpty($_user["name"]) ) {
            return null;
        }
        
        $email = $_user["email"];
        $name = $_user["name"];
        $password = self::getObjectValue($_user["password"]);
        $username = self::getObjectValue($_user["username"]);
        $instagramId = self::getObjectValue($_user["instagramId"]);
        $accessToke = self::getObjectValue($_user["accessToken"]);
        $profPict = self::getObjectValue($_user["profPict"]);
        $website = self::getObjectValue($_user["website"]);
        
        return self::userRegistration($name, $email, $password, $username, $instagramId, 
                                        $accessToke, $profPict, $website);        
    }
     ...
     * Create New Orders
     * 
     * @param int $_shoppingartId
     * @param string $_transactionId
     * @param string $_purchaseDate
     * @param int $_buyerId
     ...
     * @return array()
     */
    public static function createNewOrders($_shoppingartId, $_transactionId, $_purchaseDate, 
            $_buyerId, $_fullName, $_address, $_city, $_state, $_zip, $_country, $_status){
        
        $order = new Orders;
        $order->transaction_id = $_transactionId;
        $order->shoppingcart_id = $_shoppingartId;
        $order->purchasedate = $_purchaseDate;
        $order->buyer_id = $_buyerId;
        $order->fullname = $_fullName;
        $order->address = $_address;
        $order->city = $_city;
        $order->state = $_state;
        $order->zip = $_zip;
